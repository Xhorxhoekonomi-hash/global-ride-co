...
export const Route = createFileRoute("/import-germany")({
  head: () => {
    const base = buildHead({
      title: "Import a Vehicle to Germany | USA, Korea, Dubai, Canada",
      description: "Vehicle sourcing and shipping into Germany from the USA, South Korea, Dubai, and Canada — arrival at Bremerhaven or Hamburg, Zulassung registration overview, and full-service coordination.",
      path: "/import-germany",
      image: heroImg,
    });
    return {
      ...
      scripts: [
        {
          ...
          children: JSON.stringify({
            ...
            "@type": "Service",
            name: "Vehicle Import to Germany",
            description: "Sourcing, inspection, shipping, and customs-document coordination for vehicles imported into Germany from the USA, South Korea, Dubai, and Canada.",
            provider: { "@type": "Organization", name: "Alpha Worldwide", url: SITE_URL },
            areaServed: "Germany",
            url: `${SITE_URL}/import-germany`,
          }),
        },
        {
          type: "application/ld+json",
  ...
  component: ImportGermany,
});

const VEHICLES_IMPORTED = ["Trucks and SUVs from the USA", "Well-maintained sedans from South Korea", "Luxury and performance vehicles from Dubai", "Pickup trucks from Canada"];

const FAQS = [
  { q: "Which German port does my vehicle arrive at?", a: "Depending on the shipping route, vehicles typically arrive at Bremerhaven or Hamburg, Germany's two main vehicle-handling ports." },
  { q: "How does vehicle registration (Zulassung) work in Germany?", a: "German registration generally requires a TÜV inspection, proof of emissions compliance, and import documentation. Requirements vary and should always be confirmed with the local registration authority." },
  { q: "Does Germany have strict emissions requirements for imports?", a: "Germany applies specific emissions standards that can affect eligibility for standard registration. We recommend confirming compliance requirements for your specific vehicle with the local authority before purchase." },
  { q: "Can I get my vehicle inspected before shipping to Germany?", a: "Where operationally possible, we coordinate pre-purchase inspection at origin — availability depends on the seller, the auction, and access to the vehicle, and is never guaranteed for every listing." },
  { q: "Do you handle German customs clearance?", a: "We coordinate customs paperwork with licensed local agents at Bremerhaven or Hamburg. Customs values, taxes, and clearance decisions are set by the relevant customs authority, not by Alpha Worldwide." },
];

function ImportGermany() {
  const whatsappUrl = `https://wa.me/${CONTACT.whatsapp}?text=${encodeURIComponent(
    "Hello Alpha Worldwide, I'd like to import a vehicle into Germany.",
  )}`;

  return (
    <>
      <Breadcrumbs items={[{ label: "Import to Germany" }]} />
      <section className="relative isolate overflow-hidden text-white">
        <img src={heroImg} alt="Vehicle delivered as part of a European import" width={1920} height={900} className="absolute inset-0 -z-10 h-full w-full object-cover" />
        <div className="absolute inset-0 -z-10 bg-gradient-to-br from-navy/95 via-navy/85 to-graphite/90" />
        <div className="container-page flex min-h-[60vh] flex-col justify-end py-32">
          <div className="max-w-3xl animate-fade-up">
            <div className="eyebrow text-teal-glow">Import to · Germany</div>
            <h1 className="font-display mt-4 text-4xl font-bold leading-[1.05] md:text-6xl">
              Vehicle Sourcing and Shipping to Germany
            </h1>
            <p className="mt-5 max-w-2xl text-lg text-white/75">
              From the USA, South Korea, Dubai, or Canada — sourced, inspected where possible, and shipped through Bremerhaven or Hamburg.
            </p>
            <div className="mt-8 flex flex-wrap items-center gap-4">
              <a href={whatsappUrl} target="_blank" rel="noreferrer noopener" onClick={() => trackEvent("whatsapp_clicked", { source: "eu-germany-hero" })} className="btn-primary">
                <MessageCircle className="h-4 w-4" /> WhatsApp Us
              </a>
              <Link to="/calculator" onClick={() => trackEvent("calculator_started", { source: "eu-germany-hero" })} className="btn-outline-light">
        ...
        <div className="container-page py-16 md:py-20">
          <div className="grid gap-10 lg:grid-cols-2 lg:gap-16">
            <div>
              ...
              <h2 className="font-display mt-4 text-2xl font-bold text-navy">Arrival at Bremerhaven or Hamburg</h2>
              <p className="mt-3 text-slate-body">Germany's two main vehicle ports handle the majority of imported vehicles. We coordinate unloading and terminal handling at whichever port your shipment routes through.</p>
            </div>
            <div>
              <div className="grid h-11 w-11 place-items-center rounded-lg bg-teal/10 text-teal"><FileText className="h-5 w-5" /></div>
              <h2 className="font-display mt-4 text-2xl font-bold text-navy">Zulassung Registration Overview</h2>
              <p className="mt-3 text-slate-body">German registration typically requires a TÜV inspection and emissions-compliance documentation. Requirements vary and should always be confirmed with the local registration authority.</p>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="container-page py-16 md:py-20">
          ...
          <div className="grid gap-10 mt-10 lg:grid-cols-2 lg:gap-16">
            ...
            <div>
              ...
              <p className="mt-2 text-sm text-slate-body">Once cleared at the port, we coordinate onward delivery anywhere in Germany.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="section-dark">
        <div className="container-page grid gap-12 py-20 md:py-24 lg:grid-cols-2 lg:gap-16">
          <div>
            ...
            <p className="mt-5 max-w-md text-white/70">We'll confirm feasibility and return an itemised quote to your German destination.</p>
          </div>
          <QuoteForm variant="full" onDark source="eu-germany" />
        </div>
      </section>
    </>
  );
