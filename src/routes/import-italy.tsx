...
export const Route = createFileRoute("/import-italy")({
  head: () => {
    const base = buildHead({
      title: "Import a Vehicle to Italy | USA, Korea, Dubai, Canada",
      description: "Vehicle sourcing and shipping into Italy from the USA, South Korea, Dubai, and Canada — arrival at Genoa, PRA registration overview, and full-service coordination.",
      path: "/import-italy",
      image: heroImg,
    });
    return {
      ...
      scripts: [
        {
          ...
          children: JSON.stringify({
            ...
            "@type": "Service",
            name: "Vehicle Import to Italy",
            description: "Sourcing, inspection, shipping, and customs-document coordination for vehicles imported into Italy from the USA, South Korea, Dubai, and Canada.",
            provider: { "@type": "Organization", name: "Alpha Worldwide", url: SITE_URL },
            areaServed: "Italy",
            url: `${SITE_URL}/import-italy`,
          }),
        },
        {
          type: "application/ld+json",
  ...
  component: ImportItaly,
});

const VEHICLES_IMPORTED = ["SUVs and trucks from the USA", "Compact sedans from South Korea", "Luxury and sports vehicles from Dubai", "SUVs from Canada"];

const FAQS = [
  { q: "Which Italian port does my vehicle arrive at?", a: "Vehicles typically arrive at the Port of Genoa, one of Italy's principal ports for vehicle imports." },
  { q: "How does vehicle registration work in Italy (PRA)?", a: "Italian registration is recorded through the PRA (Public Vehicle Registry) and generally requires customs clearance, technical conformity checks, and import documentation. Requirements vary and should always be confirmed with the local registration authority." },
  { q: "Does Italy have specific conformity requirements for imported vehicles?", a: "Yes — vehicles typically need to demonstrate technical conformity with EU standards as part of the registration process. We recommend confirming current requirements for your specific vehicle with the local authority before purchase." },
  { q: "Can I get my vehicle inspected before shipping to Italy?", a: "Where operationally possible, we coordinate pre-purchase inspection at origin — availability depends on the seller, the auction, and access to the vehicle, and is never guaranteed for every listing." },
  { q: "Do you handle Italian customs clearance?", a: "We coordinate customs paperwork with licensed local agents at Genoa. Customs values, taxes, and clearance decisions are set by the relevant customs authority, not by Alpha Worldwide." },
];

function ImportItaly() {
  const whatsappUrl = `https://wa.me/${CONTACT.whatsapp}?text=${encodeURIComponent(
    "Hello Alpha Worldwide, I'd like to import a vehicle into Italy.",
  )}`;

  return (
    <>
      <Breadcrumbs items={[{ label: "Import to Italy" }]} />
      <section className="relative isolate overflow-hidden text-white">
        <img src={heroImg} alt="Vehicle prepared for shipping to a European destination" width={1920} height={900} className="absolute inset-0 -z-10 h-full w-full object-cover" />
        <div className="absolute inset-0 -z-10 bg-gradient-to-br from-navy/95 via-navy/85 to-graphite/90" />
        <div className="container-page flex min-h-[60vh] flex-col justify-end py-32">
          <div className="max-w-3xl animate-fade-up">
            <div className="eyebrow text-teal-glow">Import to · Italy</div>
            <h1 className="font-display mt-4 text-4xl font-bold leading-[1.05] md:text-6xl">
              Vehicle Sourcing and Shipping to Italy
            </h1>
            <p className="mt-5 max-w-2xl text-lg text-white/75">
              From the USA, South Korea, Dubai, or Canada — sourced, inspected where possible, and shipped through the Port of Genoa.
            </p>
            <div className="mt-8 flex flex-wrap items-center gap-4">
              <a href={whatsappUrl} target="_blank" rel="noreferrer noopener" onClick={() => trackEvent("whatsapp_clicked", { source: "eu-italy-hero" })} className="btn-primary">
                <MessageCircle className="h-4 w-4" /> WhatsApp Us
              </a>
              <Link to="/calculator" onClick={() => trackEvent("calculator_started", { source: "eu-italy-hero" })} className="btn-outline-light">
        ...
        <div className="container-page py-16 md:py-20">
          <div className="grid gap-10 lg:grid-cols-2 lg:gap-16">
            <div>
              ...
              <h2 className="font-display mt-4 text-2xl font-bold text-navy">Arrival at Genoa</h2>
              <p className="mt-3 text-slate-body">Genoa is one of Italy's principal vehicle-import ports. We coordinate unloading and terminal handling on arrival, whether by container or RoRo.</p>
            </div>
            <div>
              <div className="grid h-11 w-11 place-items-center rounded-lg bg-teal/10 text-teal"><FileText className="h-5 w-5" /></div>
              <h2 className="font-display mt-4 text-2xl font-bold text-navy">PRA Registration Overview</h2>
              <p className="mt-3 text-slate-body">Italian registration is recorded through the PRA and typically requires customs clearance and a technical conformity check. Requirements vary and should always be confirmed with the local registration authority.</p>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="container-page py-16 md:py-20">
          ...
          <div className="mt-10 grid gap-5 md:grid-cols-3">
            ...
            <Link to="/airfreight" className="rounded-2xl border border-border bg-card p-6 shadow-card transition-colors hover:border-teal">
              ...
              <p className="mt-2 text-sm text-slate-body">For urgent or exceptionally high-value vehicles, via our verified European airport network, including Milan Malpensa.</p>
            </Link>
          </div>
        </div>
      </section>
        ...
        <div className="container-page py-16 md:py-20">
          ...
          <div className="grid gap-10 mt-10 lg:grid-cols-2 lg:gap-16">
            ...
            <div>
              ...
              <p className="mt-2 text-sm text-slate-body">Once cleared at Genoa, we coordinate onward delivery anywhere in Italy.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="section-dark">
        <div className="container-page grid gap-12 py-20 md:py-24 lg:grid-cols-2 lg:gap-16">
          <div>
            ...
            <p className="mt-5 max-w-md text-white/70">We'll confirm feasibility and return an itemised quote to your Italian destination.</p>
          </div>
          <QuoteForm variant="full" onDark source="eu-italy" />
        </div>
      </section>
    </>
  );
