...
const SIZE_OPTIONS: { value: VehicleSize; icon: string; title: string; body: string }[] = [
  ...
  { value: "Large SUV/Truck", icon: "🚙", title: "Large SUV / Large Truck", body: "×1.5 land transport multiplier" },
  { value: "Oversized", icon: "🚛", title: "Oversized", body: "Large van, oversized pickup · ×2.0 land transport multiplier" },
];
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[1fr_minmax(0,26rem)] lg:items-start">
      <div className="space-y-8">
        {/* Step 1 — Location */}
        <StepBlock n={1} title="Search your auction location">
              ...
              >
                {location ? (
                  <span className="font-medium">
                    {LAND_TRANSPORT_TABLE[location].city} — {LAND_TRANSPORT_TABLE[location].state}
                  </span>
                ) : (
                    ...
                    {locations.map((loc) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <span className="ml-2 rounded bg-mist px-1.5 py-0.5 text-[10px] font-bold text-navy/70">{l.state}</span>
                        </CommandItem>
                      );
                    })}
                  </CommandGroup>
        ...
        {/* Step 2 — Vehicle size */}
        <StepBlock n={2} title="Vehicle size">
          ...
          {/* Step 3 — EV */}
          {size && (
            <label className="mt-4 flex cursor-pointer items-center gap-3 rounded-xl border border-border bg-card p-4">
        ...
        {/* Step 4 — Winning bid */}
        <StepBlock n={4} title="Winning bid at auction">
        ...
        {/* Step 5 — Destination */}
        <StepBlock n={5} title="Destination">
          <div className="grid gap-3 sm:grid-cols-3">
            {DESTINATIONS.map((d) => (
              <button
                key={d.value}
      ...
      {/* Result panel */}
      <div className={cn("transition-all duration-500", ready ? "opacity-100" : "pointer-events-none opacity-40")}>
        {ready && land && fees && unloading && destination ? (
          <div className="animate-fade-up rounded-2xl border border-border bg-card shadow-elegant lg:sticky lg:top-24">
            <div className="border-b border-border p-5">
              ...
              <div className="mt-1 text-sm font-semibold text-navy">
                {LAND_TRANSPORT_TABLE[location!].city}, {LAND_TRANSPORT_TABLE[location!].state} → {land.terminalName} → {destination}
              </div>
            ...
            <div className="space-y-5 p-5 text-sm">
              ...
              <ul className="space-y-1 text-[11px] leading-relaxed text-slate-body">
                ...
                <li>* Shared container — 4 cars per 40'HC container</li>
            ...
            <div className="space-y-2 border-t border-border p-5">
              <button
                onClick={onWhatsApp}
                type="button"
                className="flex w-full items-center justify-center gap-2 rounded-md py-3 text-sm font-semibold text-white"
              ...
              <a
                href="/contact"
                className="block text-center text-sm font-semibold text-teal hover:underline"
              >
                Request Full Quote →
              </a>
            </div>
          ...
          <div className="rounded-2xl border border-dashed border-border p-8 text-center text-sm text-slate-body">
            Complete location, size, bid, and destination to see your itemized cost breakdown.
          </div>
        )}
      </div>
    </div>
...
function KoreaCalculator() {
  ...
  const whatsappText = () => {
    ...
    return [
      ...
      `Vehicle: ${type}`,
      "",
      `Vehicle Price: ₩${Math.round(priceKrw).toLocaleString("en-US")} (~€${Math.round(priceEur).toLocaleString("en-US")})`,
      `Fixed Shipping Price: €${Math.round(shippingEur).toLocaleString("en-US")}`,
      "──────────────────────",
      `Estimated Total: €${Math.round(totalEur).toLocaleString("en-US")}`,
      "",
      "Please confirm and send next steps.",
      "Thank you.",
    ].join("\n");
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[1fr_minmax(0,26rem)] lg:items-start">
      <div className="space-y-8">
        <StepBlock n={1} title="Vehicle type">
          <div className="grid gap-3 sm:grid-cols-3">
            {KOREA_TYPES.map((t) => (
              <button
                key={t}
        ...
        <StepBlock n={2} title="Vehicle price">
          ...
          {priceNumber > 0 && (
            <p className="mt-1.5 text-xs text-slate-body">
              ≈ {currency === "KRW" ? `€${Math.round(priceEur).toLocaleString("en-US")}` : `₩${Math.round(priceKrw).toLocaleString("en-US")}`} (approximate rate)
            </p>
          )}
        </StepBlock>

        <StepBlock n={3} title="Destination">
          <div className="grid gap-3 sm:grid-cols-3">
            {DESTINATIONS.map((d) => (
              <button
                key={d.value}
      ...
      <div className={cn("transition-all duration-500", ready ? "opacity-100" : "pointer-events-none opacity-40")}>
        {ready && destination && type ? (
          <div className="animate-fade-up rounded-2xl border border-border bg-card shadow-elegant lg:sticky lg:top-24">
            <div className="border-b border-border p-5">
              <div className="text-xs font-bold uppercase tracking-wider text-teal">Cost Breakdown</div>
              <div className="mt-1 text-sm font-semibold text-navy">South Korea → {destination}</div>
            ...
            <div className="space-y-5 p-5 text-sm">
              <Row
                label="Vehicle Price"
                value={`₩${Math.round(priceKrw).toLocaleString("en-US")} (~€${Math.round(priceEur).toLocaleString("en-US")})`}
              />
              ...
              <div>
                ...
                <Row label="Fixed Shipping Price" value={`€${Math.round(shippingEur).toLocaleString("en-US")}`} bold />
              </div>

              <div className="rounded-xl bg-mist p-4">
                <Row label="Vehicle Price" value={`€${Math.round(priceEur).toLocaleString("en-US")}`} />
                <Row label="+ All-In Shipping" value={`€${Math.round(shippingEur).toLocaleString("en-US")}`} />
                <div className="my-2 border-t border-border" />
                <div className="flex items-baseline justify-between">
                  <span className="text-sm font-bold text-navy">Estimated Total</span>
                  <span className="font-display text-2xl font-bold text-teal">€{Math.round(totalEur).toLocaleString("en-US")}</span>
                </div>
              </div>
